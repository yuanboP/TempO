...
\begin{abstract}
Coding agents are supervised on correctness and design consistency but blind
to the cost of their own working loop: the harness reports whether a command
succeeded, never how long it took. The asymmetry is not merely an omission ---
it turns the agent's alignment tendency into negative feedback. As a codebase
grows, an agent faithfully preserves architectures that are silently
strangling its own iteration speed, re-runs 90-second test suites after
one-line edits, and rebuilds environments it already built, until a human
intervenes with exactly the missing signal: ``this is too slow.'' We call the
resulting degradation \emph{loop rot}. We first quantify the problem with a measurement
...
\textbf{(3) Alignment becomes negative feedback.} Over the life of a growing
codebase, the inner development loop --- build, test, codegen --- degrades
gradually. A time-blind agent does worse than fail to fix such
\emph{developability decay}: its supervised preference for design consistency
actively entrenches it. The existing architecture is ``correct'' --- it passed
every test, it matches the original design --- so the agent welds each new
feature onto the slow path, reinforcing it. Human engineers escape this loop
through a kind of temporal proprioception: they sit through the four-minute
compile that used to take twenty seconds, and the discomfort funds the
refactor. The agent never feels it; the human who eventually intervenes is
supplying, by hand, the missing time gradient.
...
\section{Motivation: Temporal Waste in the Wild}
...
Four observations motivate the rest of the paper. \textbf{(i)} Waste is
long-tailed and lethal: median 3.7\% of wall-clock, p90 34\%, and 18 trials
($\sim$8\% of failures) are \emph{time deaths} --- failed at budget with
$>$40\% of wall-clock in W1--W5. \textbf{(ii)} The harness's temporal
interface manufactures the median: holding GLM-4.7 fixed, Terminus~2's
declared-duration interface yields 7.2\% median waste versus 0.1\% under
Claude Code's native tool calling. \textbf{(iii)} But no interface fix removes
the tail: native-tool-calling harnesses still lose 10--70\% of wall-clock to
\emph{semantic} waste in their worst trials --- Claude Code regenerating a
million-row test fixture five times, Gemini CLI repeating a 698-second web
search that had just returned nothing --- and Gemini CLI contributes 5 of the
18 time deaths. \textbf{(iv)} Weaker models die of the clock, not the task:
failed-trial waste medians are 2.2--5.6\% for GPT-5.3-Codex and Opus 4.6
versus 13.9--15.7\% for GLM-5 and Kimi-k2.5. Full per-type and per-task
analysis appears in Appendix~\todo{ref}.

\section{\tempo{}: Making Iteration Cost Observable}
\label{sec:tempo}

\todo{THE core section --- harness design reserved for the authors. The
measurement study above earns exactly one claim: elapsed time must enter the
observation space, and the design question is how. Candidate ingredients from
the data (not yet a design): per-tool-result elapsed-time stamps kill W2/W3
mechanically; per-command-template cost memory addresses W1/W5 (the
million-row-fixture and repeated-search failures); budget state addresses time
deaths. Ablation axes and the actual interface are open.}
